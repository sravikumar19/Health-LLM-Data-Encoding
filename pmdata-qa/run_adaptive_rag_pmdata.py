...
def predict_fatigue_and_readiness():
    ...
    for item in os.listdir(args.data_path):
        if item != "participant-overview.xlsx" and item != ".DS_Store":
            ...
            answer, pred, retrieval, nonretrieval = adpative_retrieval(args.method, df, indices, args.context_length, prompt, args.target_var, args.encoding, args.retriever, args.corpus, args.output_path)
    ...
    for i in sampled_indices:
        if (not pd.isna(df[target_var][i])) and (i-context_length+1 >= 0):
            ...
            if encoding == "natural_language":
                ...
                heart_rate_image = Image.load_from_file("heart_rate_" + path)
                calories_image = Image.load_from_file("calories_" + path)
                sleep_image = Image.load_from_file("sleep_" + path)
            ...
            if check_string_exist(retrieval) == 0:
                count_nonretrieval += 1
                if encoding == "visual":
                    answer, _, _ = cot.answer(question=[question, steps_image, heart_rate_image, calories_image, sleep_image])
                else:
                    answer, _, _ = cot.answer(question=question)
            else:
                count_retrieval += 1
                if encoding == "visual":
                    answer, snippets, scores = medrag.answer(question=[question, steps_image, heart_rate_image, calories_image, sleep_image], k=4)
                else:
                    answer, snippets, scores = medrag.answer(question=question, k=4) 

            prediction = parse_score(answer)
            preds.append(prediction)
            answers.append(str(df[target_var][i]))

    return answers, preds, count_retrieval, count_nonretrieval
